...
bindings_path = os.path.dirname(os.path.abspath(__file__)) + "/../bindings/python"
sys.path.append(bindings_path)
os.environ["PYTHONPATH"] = bindings_path + (":" + os.environ["PYTHONPATH"] if "PYTHONPATH" in os.environ else "")
...
parser.add_argument('--replay-dir', help="Directory to save replays to", default="replays", required=False)
...
try:
    run_game(map_path, args.player1, args.player2, args.replay_dir, docker=args.docker)
